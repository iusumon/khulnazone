...
class Contacts_info extends Controller {
    var $contact_id;
    var $obj;
    var $data;
	...
	function index() {
            $this->check_login_status();
                if($this->session->userdata('logged_in') == TRUE){
                    $this->display_data();
                    $this->load->view('vcontacts_info', $this->data);
                } else {
	...
	function generate_id() {
            $this->check_login_status();
		$query = $this->db->select_max('id')->get('tbl_contacts');
		if ($query->num_rows() > 0) {
			$row = $query->row();
	...
	function display_data() {
            $this->check_login_status();
                ...
                $custom_field = "concat(\'<a class=\"updateBtn\" href=\"contacts_info/update_data/', tbl_contacts.id, \'\" >Update/Edit</a> \')";
                $custom_field = strip_slashes($custom_field);
                $sql = "SELECT id, name, designation, phone, mobile, fax, email, remarks, $custom_field AS Action FROM tbl_contacts ORDER BY id";
                $query = $this->db->query($sql);
                
		if ($query->num_rows() > 0) {
                        $tmpl = array ('table_open'=> '<table id="records">', 'heading_row_start' => '<thead><tr>', 'heading_row_end' => '</tr></thead><tbody>');
			$this->table->set_template($tmpl);
                        $this->table->set_heading('ID', 'Name', 'Designation', 'Phone', 'Mobile', 'Fax', 'Email', 'Remarks', 'Action');
...
//                }
	...
	function getById($id) {
            $this->check_login_status();
		$query = $this->db->where('id', $id)->limit(1)->get('tbl_contacts');
		if ($query->num_rows() > 0) {
	...
	//----------------------------------------------------------------------
        function show_report(){
            $this->check_login_status();
		$sql = "SELECT id, name, designation, phone, mobile, fax, email, remarks FROM tbl_contacts ORDER BY id";
                $query = $this->db->query($sql);
                $this->load->library('table');
		if ($query->num_rows() > 0) {
